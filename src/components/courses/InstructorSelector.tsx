...
const toTags = (instructors: Instructor[]) =>
...
export default function InstructorSelector({
  ...
  const { instructors: foundInstructors, loadInstructors } = useInstructors({
    initialStatus: ScreeningStatus.Accepted,
    initialSearch: '',
  });
  ...
  useEffect(() => {
    loadInstructors({
      ...
      search: debouncedInstructorSearch,
    });
  ...
  const addInstructor = ({ value }: { value: string }) => {
    ...
    if (!alreadyChosen(newInstructor))
  ...
  return (
    <>
      ...
      >
        {foundInstructors
          .filter((instructor) => !alreadyChosen(instructor))
          .map((instructor) => (
            <Select.Option
              key={instructor.email}
              value={JSON.stringify(instructor)}
