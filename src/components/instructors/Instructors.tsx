...
import {
  screeningTemplateAG,
  screeningTemplateIntern,
} from './screeningTemplate';
...
const Instructors = () => {
  ...
  const {
    ...
    initialStatus: ScreeningStatus.Unscreened,
    initialSearch: '',
  });
  ...
  useEffect(() => {
    loadInstructors({ screeningStatus, search: debouncedSearch });
  }, [screeningStatus, debouncedSearch]);
...
function InstructorTable({
  ...
  screeningStatus: ScreeningStatus;
  setScreeningStatus(screeningStatus: ScreeningStatus): void;
  instructors: Instructor[];
  loading: boolean;
  setSearch(search: string): void;
  setEditInstructor(instructor: Instructor): void;
}) {
  const columns = [
    {
      title: 'Nachname',
  ...
  const toScreeningStatus = (status: boolean | undefined) =>
    status === undefined
      ? ScreeningStatus.Unscreened
      : status
  ...
  const rowClassName = (record: Instructor) => {
    ...
    if (
      toScreeningStatus(record.__instructorScreening__?.success) ===
      screeningStatus
    )
      return '';

    if (record.__instructorScreening__?.success === true) return 'green';

    if (record.__instructorScreening__?.success === false) return 'red';
...
function UpdateInstructor({
  ...
  const screening = instructor.__instructorScreening__ ?? {
    comment: instructor.module ? screeningTemplateIntern : screeningTemplateAG,
    knowsCoronaSchoolFrom: '',
    success: null,
  };
  const history = useHistory();

  const [phone] = useState(instructor.phone);
  ...
  const [knowscsfrom, setKnowscsfrom] = useState(
        ...
        {isEditMode && (
          <Space size="small">
            {showSaveButton && (
              <Button
                onClick={() =>
                  update(instructor.__instructorScreening__!.success)
                }
              >
                Speichern
              </Button>
  ...
  const customDetails = () => {
    ...
    const studentField = () => {
      ...
      return (
        <Card>
          {!isEditMode && isStudent && <Text strong>{TEXT}</Text>}
      ...
      return (
        <Card
          title={
            <>
              {' '}
              <FileTextOutlined /> Feedback vom Kursleiter{' '}
            </>
          }
        >
